...
Vec2 Force::GenerateDragForce(const Body& body, float k) {
    ...
    if (body.velocity.MagnitudeSquared() > 0) {
        dragForce = body.velocity.UnitVector() * -1; // 反向
        dragForce *= k * body.velocity.MagnitudeSquared(); // Fd = -k * v^2
    }
    return dragForce;
}
...
Vec2 Force::GenerateFrictionForce(const Body& body, float k) {
    ...
    if (body.velocity.MagnitudeSquared() > 0) {
        frictionForce = body.velocity.UnitVector() * -1; // 反向
...
Vec2 Force::GenerateSpringForce(const Body& body, Vec2 anchor, float restLength, float k) {
    Vec2 d = body.position - anchor; // 计算粒子与锚点之间的距离向量
...
Vec2 Force::GenerateSpringForce(const Body& a, const Body& b, float restLength, float k) {
    Vec2 d = a.position - b.position; // 计算两个粒子之间的距离向量
    float displacement = d.Magnitude() - restLength; // 计算弹簧的伸长量（当前距离 - 休息长度）
    Vec2 springDirection = d.UnitVector(); // 计算弹力的方向
    float springMagnitude = (-k) * displacement; // 计算弹力的大小，使用胡克定律 F = -k * x，其中 x 是弹簧的伸长量（当前距离 - 休息长度）
    Vec2 springForce = springDirection * springMagnitude; // 弹力的方向与伸长量相反
